...
const CATEGORIES = [
  ...
  { value: "suv",      label: "SUV",           size: "L",  desc: "Peugeot 3008, RAV4…", range: "470–500 cm", w: 96  },
  { value: "grand",    label: "Grand gabarit", size: "XL", desc: "Touareg, Tesla X…",   range: "> 500 cm",   w: 108 },
];
...
export default function SetupVehiclePage() {
  ...
  function handleVehicleSelect(
    ...
    if (!newModel) return; // model reset (brand changed)

    if (localLengthCm) {
      /* Données locales → on s'en sert directement */
      setLengthCm(localLengthCm);
      setCategory(getCategory(localLengthCm));
      setAutoDetected(true);
    } else if (newMake && newModel) {
      /* Modèle inconnu → on essaie carqueryapi */
  ...
  /* Sync catégorie ↔ longueur (slider) */
        ...
        <div className="flex-1 px-5 space-y-4 pb-10">
          <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 space-y-4">
            ...
            <VehiclePicker
              ...
              onModelChange={(m, len) => handleVehicleSelect(make, m, len)}
            />
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex : 2021"
                className="w-full bg-gray-50 border-2 border-transparent rounded-2xl px-4 py-3 text-sm outline-none focus:border-[#22956b] focus:bg-white transition font-medium"
              />
            </div>
          </div>
            ...
            <>
              {autoDetected ? (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                  ...
                  <p className="text-xs font-semibold text-emerald-700">
                    Dimensions détectées · ajuste si besoin
                  </p>
                </div>
              ) : (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border border-amber-100 rounded-2xl">
                  ...
                  <p className="text-xs font-semibold text-amber-700">
                    Dimensions non trouvées — choisis ton gabarit
                  </p>
                </div>
              )}
              ...
              <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-black text-gray-900">Longueur</p>
                ...
                <input
                  type="range" min={300} max={600} step={5} value={lengthCm}
                  onChange={(e) => handleLengthChange(Number(e.target.value))}
                  className="w-full accent-[#22956b] cursor-pointer"
                />
                <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                  <span>300 cm</span><span>450 cm</span><span>600 cm</span>
                </div>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition text-left ${
                        active ? "border-[#22956b] bg-[#22956b]/5" : "border-gray-100 bg-white"
                      }`}
                    >
        ...
        <div className="flex-1 px-5 space-y-4 pb-10">
          ...
          <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-gray-400 mb-3">
              Aide à identifier ta voiture parmi d&apos;autres similaires.
              Jamais visible par les autres.
            </p>
            <input
              type="text"
              value={plate}
